...
jv_laplacian_unroll_jax = jax.jit(jax.vmap(laplacian_9_with_1_jax))

@jax.jit
def gray_scott_jax_uv(UV,UVV, Du, Dv, f, k, dt):
    
    UVV = UV[0] * UV[1] * UV[1]
    LAP = jv_laplacian_unroll_jax(UV)
    UV = UV.at[0].add((Du * LAP[0]) - UVV + f * (1 - UV[0]) * dt)
    UV = UV.at[1].add((Dv * LAP[1]) + UVV - UV[1] * (f + k) * dt)
    
    return UV

def gray_scott_vmap(U, V, Du, Dv, f, k, dt, nimages, dsteps):
    ...
    for idx in range(nimages):
        for _ in range(dsteps):
            UV = gray_scott_jax_uv(UV,UVV, Du, Dv, f, k, dt)
...
if __name__ == "__main__":
    if True:
        ...
        nb_frame = 100
        gsc.grayscott_main(gray_scott_vmap, gs_pars, U, V, nb_frame)
