...
def test_jax_ite():
    print("=========== test_jax_ite")
    j_au = jnp.arange(16).reshape(4, 4).astype(jnp.float32)
    j_av = jnp.ones(16).reshape(4, 4).astype(jnp.float32)
...
def test_jax_inplace():
    ...
    j_au = jnp.arange(16).reshape(4, 4).astype(jnp.float32)
    j_av = jnp.ones(16).reshape(4, 4).astype(jnp.float32)
    # jit_inplace = jit(inplace)
    inplace(j_au, j_av)
    print("after inplace")
    print(j_au)
    jit_inplace_add = jit(inplace_add)
    j_au, j_av= jit_inplace_add(j_au, j_av)
    print("after inplace_add")
    print(j_au)
...
def test_kernel_nip():
    ...
    ou, ov = jit_grayscott_kernel_nip(
        j_Lu, j_Lv, j_u[1:-1, 1:-1], j_v[1:-1, 1:-1], Du, Dv, F, k, delta_t
    )
    plot_array(j_u, "array U")
